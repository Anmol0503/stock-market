...
# ---- publish (best-effort) ----
# Publish ONLY the Mac-owned files: the lesson artifacts. Everything else under dashboard/ (news, status,
# reels) is the CLOUD's — committing our local copies would collide with its hourly pushes on rebase. So we
# stage just these, then discard any other working-tree changes before rebasing, guaranteeing a conflict-free
# fast-forward.
